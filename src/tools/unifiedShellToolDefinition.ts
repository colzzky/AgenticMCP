...
export function getUnifiedShellToolDefinition() {
  return {
    type: 'function',
    name: 'shell',
    description: 'Execute a shell command with arguments. Only allowed commands can be executed for security purposes.',
    parameters: {
      type: 'object',
      properties: {
        command: {
          type: 'string',
          description: 'The shell command to execute. Must be one of the allowed commands.',
          enum: [...SHELL_COMMANDS]
        },
        args: {
          type: 'array',
...
// This is a helpful reference for when the command is allowed
export const shellCommandDescriptions: Record<string, string> = {
