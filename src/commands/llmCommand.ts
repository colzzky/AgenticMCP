...
export class LLMCommand extends BaseCommand {
  ...
  async execute(context: CommandContext, ...args: unknown[]): Promise<CommandOutput> {
    ...
    try {
      ...
      const provider = globalThis.providerFactory.getProvider(providerName as any);
...
Usage:
  agenticmcp llm [options] <prompt> [file paths...]

Description:
  Interact with LLMs, using file paths as context.
  Any file paths provided will be loaded and added as context to the prompt.

Options:
  -p, --provider <provider>  LLM provider to use (default: openai)
  -m, --model <model>        Model to use with the provider

Examples:
  agenticmcp llm "Summarize these files" file1.txt file2.md
  agenticmcp llm --provider anthropic --model claude-3-opus "Explain this code" src/index.ts
  agenticmcp llm "Find bugs in" ./*.js
`;
  }
}
