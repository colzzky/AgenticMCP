/**
 * Unit tests for localCliToolDefinitions
 * Tests the CLI tool schema definitions and validation
 */
import { describe, it, expect } from '@jest/globals';
import { getLocalCliToolDefinitions, validateToolDefinitions } from '../../src/tools/localCliToolDefinitions.js';
import type { Tool } from '../../src/core/types/provider.types.js';

describe('localCliToolDefinitions', () => {
  describe('getLocalCliToolDefinitions', () => {
    let toolDefinitions: Tool[];
    
    beforeEach(() => {
      toolDefinitions = getLocalCliToolDefinitions();
    });
    ...
    it('should define write_file tool with correct parameters', () => {
      ...
      expect(tool?.parameters.properties.allowOverwrite).toHaveProperty('type', 'boolean');
      expect(tool?.parameters.required).toContain('path');
    ...
    it('should define search_codebase tool with correct parameters', () => {
      ...
      expect(tool?.parameters.properties.recursive).toHaveProperty('type', 'boolean');
  ...
  describe('validateToolDefinitions', () => {
    it('should validate valid tool definitions for OpenAI', () => {
      const tools = getLocalCliToolDefinitions();
      const result = validateToolDefinitions(tools, 'openai');
    ...
    it('should validate valid tool definitions for Anthropic', () => {
      const tools = getLocalCliToolDefinitions();
      const result = validateToolDefinitions(tools, 'anthropic');
    ...
    it('should validate valid tool definitions for Google', () => {
      const tools = getLocalCliToolDefinitions();
      const result = validateToolDefinitions(tools, 'google');
    ...
    it('should identify tools with invalid type', () => {
      ...
      expect(result.messages.some(msg => msg.includes('invalid type for OpenAI'))).toBe(true);
    });

    it('should identify tools with invalid parameters type', () => {
      const invalidTools: Tool[] = [
        {
          type: 'function' as const,
          name: 'valid_tool',
        ...
        },
        {
          type: 'function' as const,
          name: 'invalid_tool',
          description: 'A tool with invalid parameters type',
          parameters: {
            // @ts-expect-error Intentionally invalid for testing
            type: 'array',
            properties: {
      ...
      expect(result.valid).toBe(false);
      expect(result.invalidTools).toContain('invalid_tool');
      expect(result.messages.some(msg => msg.includes('parameters must have type \'object\''))).toBe(true);
    });
